...
contract GEMNFTs is ERC721, Ownable {
    ...
    */
    ...
    function mintGEMNFTAUTH(string memory _id, address _receiver) public {
        require(msg.sender == mintingAddress, "No external Minting");
    ...
    */
    ...
    /**
    * Burn the NFT (only KYC verified users can burn an NFT)
    */

    function burn(uint256 tokenId) public payable {
        require(burnIsActive, "Burn not active");
        require(msg.value == burnFee, "Fee error");
    ...
    */
    ...
    /**
    * Function to create a GeM NFT category
    */

    function setGemCategory(string memory _id, string memory _URI, uint256 _price, uint256 _supply, uint256 _fee) public adminRequired {
        require(gemCategories[_id].status == false);
        gemCategories[_id].URI = _URI;
        gemCategories[_id].price = _price;
        gemCategories[_id].supply = _supply;
        gemCategories[_id].fee = _fee;
        gemCategories[_id].status = true;
    }
    ...
    */
    ...
    function updateCategoryStatus(string memory _id) public adminRequired {
        gemCategories[_id].status = false;
    }

    /**
    * Function to update the fee of a category
    */

    function updateCategoryFee(string memory _id, uint256 _fee) public adminRequired {
        gemCategories[_id].fee = _fee;
    }

    /**
    * Function to update the burn fee
    */

    function updateBurnFee(uint256 _fee) public adminRequired {
        burnFee = _fee;
    }
    ...
    */
    ...
    /**
    * Function to return the tokenURI for a specific token
    */

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        return string(abi.encodePacked(tURI[tokenId]));
    }

    /**
    * Function to return the URI, the price and status of predefined data
    */

    function retrieveCategoryData(string memory _id) public view returns (string memory, uint256, uint256, uint256, uint256, bool) {
        return (gemCategories[_id].URI, gemCategories[_id].price, gemCategories[_id].counter, gemCategories[_id].supply, gemCategories[_id].fee, gemCategories[_id].status);
    }

    /**
    * Function to return the category price
    */
